...
public:
    bool isPalindrome(ListNode* head) {
        ...
        ListNode *reversed = reverseNodeList(slow);

        bool answer = isSame(head, reversed);

        // 还原
        pre_slow->next = reverseNodeList(reversed);

        return answer;
    }

    bool isSame(ListNode *l, ListNode *r){
        ListNode *p, *q;
        for (p = l, q = r; p!= nullptr && q!= nullptr; p = p->next, q = q->next)
