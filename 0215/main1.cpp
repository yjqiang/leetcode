...
public:
    int findKthLargest(vector<int>& nums, int k) {
        return quickSort(nums, k, 0, (int)nums.size()-1);
    }

    // k 是说，在 nums[start, start+1, ..., end] 这几个数字中，想查找第 k 大的数字
    int quickSort(vector<int> &nums, int k, int start, int end) {
        ...
        while(low < high){
            for (; low < high && nums[high] >= guard; --high);
            nums[low] = nums[high];
            for (; low < high && nums[low] <= guard; ++low);
            nums[high] = nums[low];
        }
