...
public:
    int oddEvenJumps(vector<int>& arr) {
        ...
        // 如果有相同的数值，仅保留最近的那个 index（题目要求）
        map<int, int> values;
        values[arr[n-1]] = n-1;

        for (int i = n-2; i >= 0; i--){
            auto it1 = values.lower_bound(arr[i]);
            auto it2 = values.upper_bound(arr[i]);

            // it1 表示 大于等于 arr[i] 的最小值
            dp[i][1] = (it1 != values.end()) && (dp[it1->second][0]);

            // it2 表示 大于 arr[i] 的最小值，it2-- 表示小于等于 arr[i] 的最大值
            dp[i][0] = (it2 != values.begin()) && (dp[(--it2)->second][1]);
