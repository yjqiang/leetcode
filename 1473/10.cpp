...
public:
    ...
    int minCost(vector<int>& houses, vector<vector<int>>& cost, int m, int n, int target) {
        ...
        // dp[i][color][t] 拆分子问题有一个，dp[i-1][not same color][t-1]
        // 这是个 O(n) 的遍历（固定 i 和 t，遍历 color）

        // 我们可以存储某个 i （仅存一个 i 就可以了，因为 dp[i] 仅依赖 dp[i-1]）时候，针对 某个 t，存储 所有 color 的最小和次小 cost
        // 并记录最小是哪个颜色
        // 这样在求 min(dp[i-1][not same color][t-1]) （遍历 color）时候，
        //      如果 color == first_idx，则次小值对应的颜色一定不是 color（最大值次小值不一个元素）， min(dp[i-1][not same color][t-1]) = 次小
        //      否则 min(dp[i-1][not same color][t-1]) = 最小
        // leasts[i][t] = (first,first_idx,second)
        vector<vector<tuple<long long, int, long long>>> leasts(m+1, vector<tuple<long long, int, long long>>(target+1, {INT_MAX, -1, INT_MAX}));
        ...
        for (int color = 1; color <= n; ++color) {
            ...
            auto&& [first, first_idx, second] = leasts[1][1];
            if (dp[1][color][1] < first) {
                second = first;
                first = dp[1][color][1];
                first_idx = color;
            }
            else if (dp[1][color][1] < second) {
                second = dp[1][color][1];
            }
            ...
            for (int color = 1; color <= n; ++color) {
                ...
                for (int t = 1; t <= target && t <= i; ++t) {
                    if (houses[i - 1] == 0 || houses[i - 1] == color) {
                        ...
                        // 和 houses[i-2] 颜色不同
                        if (t > 1) {
                            auto &&[first, first_idx, second] = leasts[i - 1][t - 1];
                            if (first_idx != color)
                                min_cost = min(min_cost, first);
                            else
                                min_cost = min(min_cost, second);
                        }
                    ...
                    auto&& [first, first_idx, second] = leasts[i][t];
                    if (dp[i][color][t] < first) {
                        second = first;
                        first = dp[i][color][t];
                        first_idx = color;
                    }
                    else if (dp[i][color][t] < second) {
                        second = dp[i][color][t];
                    }
                }
            }
