...
public:
    bool isValidBST(TreeNode* root) {
        ...
        long long pre_val = INT64_MIN;

        my_stack.emplace_back(root, 0);

        while (!my_stack.empty()){
            pair<TreeNode*, int> cur = my_stack.back();
            my_stack.pop_back();
            ...
            if (cur.second == 0){
                ...
                if (pre_val >= cur.first->val)
                    return false;
//                cout << pre_val << endl;
                pre_val = cur.first->val;
            }
        }
        return true;
    }
...
int main() {
    ...
    vector<int> root_ = {5,1,4,null,null,3,6};
    TreeNode *root = createTree(root_);
    printTree(root, nullptr, false);
    int n = 3;
    int answer = Solution().isValidBST(root);
    cout << answer << endl;
}