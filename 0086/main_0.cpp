...
struct ListNode* partition(struct ListNode* head, int x){
    struct ListNode* less_list = (struct ListNode*)malloc(sizeof(struct ListNode));
    struct ListNode* fake = (struct ListNode*)malloc(sizeof(struct ListNode));
    fake->next = head;
    head = fake;


    struct ListNode* p, *pre, *less_list_p;

    for (p=head->next, pre=head, less_list_p=less_list; p;){
        if (p->val < x){
            less_list_p->next = p;
            pre->next = p->next;
            p=p->next;
